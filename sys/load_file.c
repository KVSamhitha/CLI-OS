...
void load_file() {
	 ...
	 clear_vir_addr = (uint64_t *)((uint64_t)file_cr3 | (uint64_t)0xffffffff80000000);
	 ...
	 task[parse_taskid].task_id = parse_taskid;
         ...
         task[parse_taskid].task_ustack_top = (uint64_t)&(task[parse_taskid].task_ustack[ulimit/8]);
